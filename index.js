...
function getUppercaseNames(items) {
  // TODO: use `map`
  const itemsUpperCase = items.map((element) => element.name.toUpperCase());
...
function getItemById(items, id) {
  // TODO: use `find`
  const findItemId = items.find((element) => element.id === id);
  return findItemId ? findItemId.name : undefined;
}
...
function getItemPriceByName(items, name) {
  ...
  for (let i = 0; i < items.length; i++) {
    if (name === items[i].name) {
      const itemsPrice = items[i].price;
      return itemsPrice;
    }
  }
...
function getItemsByCategory(items, category) {
  ...
  const itemNames = itemsCategory.map((element) => element.name);
  return itemNames;
}
...
function calculateTotalPrice(items) {
  // TODO: use `reduce`
  const itemPrice = items.reduce((accumulator, currentValue) => {
